...
class HookDocsGenerator
{
    ...
    {
        $files = [];
        $files['Core Classes'] = array_unique(
            self::getFiles('*.php', GLOB_MARK, self::SOURCE_PATH)
        );
    ...
    protected static function getFileURL(array $file): string
    {
        ...
        $url = str_replace('.php', '.html#source-view.' . $file['line'], $path);
        $url = str_replace(['_', '/'], '-', $url);

        return '../files/' . strtolower($url);
    }
    ...
    protected static function getFiles($pattern, $flags = 0, $path = '')
    {
        ...
        if (is_array($paths)) {
            foreach ($paths as $p) {
                if (str_contains($p, './app/app')) {
                    // echo "Found './app/app' in string!\n";
                    // continue;
                }
    ...
    protected static function getHooks(array $files_to_scan): array
    {
        ...
        foreach ($files_to_scan as $heading => $files) {
            ...
            foreach ($hooks_found as $hook => $details) {
                if (!strstr($hook, 'campaignbay') && !strstr($hook, 'product') && !strstr($hook, 'wc_')) {
    ...
    protected static function getJSOutput(array $hook_list): string
    {
        ...
        foreach ($hook_list as $heading => $hooks) {
            foreach ($hooks as $hook => $details) {
                ...
                foreach ($details['files'] as $file) {
                    $summary .= ' located in ' . str_replace('campaignbay/', '', $file['path']) . ': ' . $file['line'];

                    $output .= ',{';
                    $output .= 'fqsen: "' . $name . '",';
                    $output .= 'name: "' . $hook . '",';
                    $output .= 'summary: "' . $summary . '",';
                    // $output .= 'url: "' . str_replace('../', 'https://devs.wpanchorbay.com', self::getFileURL($file)) . '"';
                    $output .= 'url: "' . self::getFileURL($file) . '"';
                    $output .= '}';
                }
            }
        }
